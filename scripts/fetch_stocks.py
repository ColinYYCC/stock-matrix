...
# AKShare 的 request_with_retry 函数发请求时不带 Referer 头，
# 东方财富服务器会因此断开连接（RemoteDisconnected）。
# 这里在 import akshare 之前 monkey-patch requests.Session.get，
# 给所有发往东方财富/新浪/同花顺的请求自动带上 Referer 和 User-Agent。
_original_session_get = requests.Session.get


def _patched_session_get(self, url, **kwargs):
    headers = kwargs.get("headers") or {}
    if "eastmoney.com" in url or "sinajs.cn" in url or "10jqka.com.cn" in url:

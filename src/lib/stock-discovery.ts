...
const INDUSTRY_TO_BOARD: Record<string, string> = {
  ...
  // ===== 旧版/别名兜底映射（subboards.json 遗留数据兼容）=====
  国防军工: "国防航天",
  医药生物: "医药制药",
  计算机: "AI与软件",
  建筑装饰: "地产基建",
...
const ROBOT_STOCKS: Record<string, string> = {
  ...
  // === 核心零部件 ===
  "688017.SH": "机器人",   // 绿的谐波(减速器)
  "920593.BJ": "机器人",   // 鼎智科技(执行器)
  "300124.SZ": "机器人",   // 汇川技术(伺服)
...
export function parseClistStocks(payload: unknown | unknown[]): StockSnapshot[] {
  ...
  for (const singlePayload of payloads) {
    ...
    for (const item of diff) {
      ...
      // 机器人子板块覆盖：把机器人公司从「自动化设备」拆出来
      if (fullCode in ROBOT_STOCKS) {
        subBoardName = ROBOT_STOCKS[fullCode];
      }
